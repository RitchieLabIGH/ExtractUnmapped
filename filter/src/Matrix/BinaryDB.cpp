...
bool BinaryDB::create(std::string file, int64_t prefix_size) {
	...
	std::string line = getLineFromFile(file, 0);
	...
	bool reading = getline(istr, line) ? true : false;
	if (reading) {
		boost::split(content, line, boost::is_any_of("\t "));
		uint32_t count = std::stoll(content[1]);
		Kmer kmer(content[0]);
		...
		while (reading) {
			boost::split(content, line, boost::is_any_of("\t "));
			count = std::stoll(content[1]);
			prefix = Prefix(content[0].substr(0, prefix_size));
			suffix = Suffix(content[0].substr(prefix_size));
			...
			tot_count += count;
			tot_suffix++;
			reading = getline(istr, line) ? true : false;
		}
...
bool BinaryDB::open(std::string file) {
	...
	buffer_prefix_size = std::ceil((buffer_size * unit_prefix_binary)/10); /// read all the prefixes
...
int64_t BinaryDB::bestPrefixSize(std::string file) {
	uint64_t tot_kmers = countFileLines(file);
